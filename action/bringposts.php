...
if (!isset($_GET['u'])) {
    ...
    if ($group == 0) {
    	$sql = "SELECT * FROM posts WHERE ((added_by IN ($yourfollowing_quoted) AND posted_to = '0') OR (added_by = '$usernameid' AND posted_to = '0' AND post_group = '0') AND (post_group = '0')) ORDER BY id DESC LIMIT $offset,20";
    }
    else {
