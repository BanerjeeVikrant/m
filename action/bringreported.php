...
$reported_ids = $conn->query("SELECT post_id FROM report");
$getposts = array(0);

for ($i=0;$i++;$i<count($reported_ids)) {
  $id = $reported_ids[$i];
  $p = $conn->query("SELECT * FROM posts WHERE id = $id");
  $getposts = array_push($getposts, $p);
}
